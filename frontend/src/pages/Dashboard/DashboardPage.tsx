...
export default function DashboardPage() {
  ...
  const alertColumns = [
    ...
    { title: 'Time', dataIndex: 'detected_at', key: 'detected_at', render: formatDateShort },
  ];
  ...
  return (
    <>
      <Row gutter={[16, 16]}>
        <Col xs={12} sm={6}>
          <Card>
            <Statistic title="Firing Alerts" value={firingCount} prefix={<AlertOutlined />} valueStyle={{ color: firingCount > 0 ? '#ff4d4f' : '#52c41a' }} />
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card>
            <Statistic title="Acknowledged" value={acknowledgedCount} prefix={<WarningOutlined />} valueStyle={{ color: '#faad14' }} />
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card>
            <Statistic title="Open Incidents" value={openIncidents} prefix={<FileTextOutlined />} />
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card>
            <Statistic title="Critical" value={criticalIncidents} prefix={<WarningOutlined />} valueStyle={{ color: criticalIncidents > 0 ? '#ff4d4f' : '#52c41a' }} />
          </Card>
        </Col>
      </Row>
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
        ...
        <Col xs={24} lg={12}>
          <Card title="Open Incidents" size="small">
            <Table
